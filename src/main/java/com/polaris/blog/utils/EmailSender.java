...
public class EmailSender {
    ...
    private EmailSender() {
        EmailSender.config(EmailSender.SMTP_QQ(false), "polaris424@foxmail.com", "qwogsbuoztdxeahd");
    }

    public static void sendRegisterVerifyCode(String code,String emailAddress) throws MessagingException {
        EmailSender.subject("polaris博客系统注册验证码")
                .from("polaris博客")
                .text("【polaris博客】你的邮箱验证码：" + code
                        + "，有效时间10分钟，如非本人操作，请忽略此邮件。")
                .to(emailAddress)
                .send();
    }
